import { ThesisSubmission, User, Student } from '../../../models/index.js';
...
export const listForm = async (req, res) => {
  try {
    const forms = await ThesisSubmission.findAll({
...
export const createForm = async (req, res) => {
  try {
    ...
    const form = await ThesisSubmission.create({
...
export const loadForm = async (req, res) => {
  try {
    ...
    const form = await ThesisSubmission.findByPk(form_id, {
      include: [{ model: Student, as: 'student', include: ['user', 'department'] }]
    });
