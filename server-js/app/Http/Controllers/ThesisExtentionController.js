import { ThesisExtention, User, Student } from '../../../models/index.js';
...
export const listForm = async (req, res) => {
  try {
    const forms = await ThesisExtention.findAll({
...
export const createForm = async (req, res) => {
  try {
    ...
    const form = await ThesisExtention.create({
...
export const loadForm = async (req, res) => {
  try {
    ...
    const form = await ThesisExtention.findByPk(form_id, {
      include: [{ model: Student, as: 'student', include: ['user', 'department'] }]
    });
