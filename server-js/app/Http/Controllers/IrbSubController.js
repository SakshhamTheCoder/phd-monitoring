import { IrbSubForm, User, Student } from '../../../models/index.js';
...
export const listForm = async (req, res) => {
  try {
    const forms = await IrbSubForm.findAll({
...
export const createForm = async (req, res) => {
  try {
    ...
    const form = await IrbSubForm.create({
...
export const loadForm = async (req, res) => {
  try {
    ...
    const form = await IrbSubForm.findByPk(form_id, {
      include: [{ model: Student, as: 'student', include: ['user', 'department'] }]
    });
...
export const submit = async (req, res) => {
  try {
    ...
    // TODO: Implement complete IRB submission form logic
...
export const bulkSubmit = async (req, res) => {
  try {
    ...
    if (!['hod', 'adordc', 'dordc', 'external', 'doctoral'].includes(user.current_role?.role)) {
      return res.status(403).json({ message: 'You are not authorized' });
    }
