...
class SupervisorAllocation extends Model {
    ...
    getCommonFields() {
        return {
            // ... assuming standard common fields list as seen in other files
    ...
    async fullCommonForm(user, extraData = []) {
        // ... (Repeating standard trait logic implementation)
        ...
        const supervisors = await student.getSupervisors(); // This might be empty for allocation form
        ...
        const commonJSON = {
            ...
            semester: this.semester, // wait, semester property exists in trait? Trait code: 'semester' => $this->semester. Assuming it exists on model.
            // On SupervisorAllocation PHP model, 'semester' isn't fillable but might be on table. Trait assumes it.
    ...
    async fullForm(user) {
        ...
        // Complex mappings
        const student = await this.getStudent();
        // broad_area_specialization relation
        const broadAreas = await student.getBroad_area_specialization({ include: ['specialization'] });

        const broadAreaOfResearch = broadAreas.map(ba => ba.specialization ? ba.specialization.broad_area : null);
        const broadAreaOfResearchId = broadAreas.map(ba => ba.specialization ? ba.specialization.id : null);

        // Fetch Faculty details for preferences and supervisors
        // prefrences and supervisors are JSON arrays of faculty_code strings
        // We need to fetch models.Faculty
        const { Faculty } = sequelize.models; // Avoiding circular dependency if possible

        const mapFaculty = async (codes) => {
            if (!codes || !Array.isArray(codes)) return [];
            return await Promise.all(codes.map(async (code) => {
                // Faculty::where('faculty_code', $code)->first()
                ...
                return {
                    ...
                    email: faculty.user ? faculty.user.email : null,
                    supervised_campus: faculty.supervised_campus, // only for 'supervisors' map but safe to include
                    supervised_outside: faculty.supervised_outside // only for 'supervisors' map
                };
            }));
        };

        const preferencesData = await mapFaculty(this.prefrences);
        const supervisorsData = await mapFaculty(this.supervisors);

        return Object.assign(commonJSON, {
            broad_area_of_research: broadAreaOfResearch,
            broad_area_of_research_id: broadAreaOfResearchId,
            prefrences: preferencesData.filter(d => d), // Filter nulls
            supervisors: supervisorsData.filter(d => d)
        });
    }

    static associate(models) {
        SupervisorAllocation.belongsTo(models.Student, { foreignKey: 'student_id', targetKey: 'roll_no', as: 'student' });
    }
}

SupervisorAllocation.init({
    // Common fields
    ...
    // Specific fields
    prefrences: DataTypes.JSON, // 'prefrences' in PHP
    supervisors: DataTypes.JSON,
}, {
    sequelize,
    modelName: 'SupervisorAllocation',
    tableName: 'supervisor_allocation_form',
    underscored: true,
});
