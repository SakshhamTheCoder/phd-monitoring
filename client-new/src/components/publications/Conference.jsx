...
const Conference = ({callback,updateValue,data={}}) => {
    ...
    return (
        <>
            ...
            <GridContainer elements={[
                <InputSuggestions apiUrl={apiCountries} label={"Country"} hint={"Country"} initialValue={data.country} onSelect={(value)=>{body.country=value.name; body.country_code=value.code; setBodyValue("country",value.name)}}  />,
                <InputSuggestions apiUrl={apiStates} label="State" hint={"State"} initialValue={data.state} onSelect={(value)=>{body.state=value.name; body.state_code=value.code;  setBodyValue("state",value.name)}} body={body} />,
                <InputSuggestions apiUrl={apiCities} label="City" hint={"City"} initialValue={data.city} onSelect={(value)=>{body.city=value.name;  setBodyValue("city",value.name)}}  body={body}/>,
            ]}/>
