...
class SupervisorChangeFormController extends Controller {
    ...
    public function listForm(Request $request, $student_id=null)
    {
       ...
       // The list must match the detail view (loadForm below): faculty,
       // adordc, doctoral and external are not steps in this form's chain and
       // loadForm 403s all of them, so they should never see a populated
       // queue either.
       if (!in_array($user->current_role->role, ['student', 'hod', 'phd_coordinator', 'dordc', 'dra', 'director', 'admin'], true)) {
           return response()->json(['message' => 'You do not have permission to view supervisor change forms. Contact your administrator if you believe this is a mistake.'], 403);
       }
